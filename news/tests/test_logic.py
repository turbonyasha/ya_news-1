...
class TestCommentCreation(TestCase):
    ...
    def test_anon_cant_create_comment(self):
        """Проверка POST-запросов на добавление комментариев, анон."""
    ...
    def test_user_can_create_comment(self):
        """Проверка POST-запросов на добавление комментариев, юзер."""

        respounse = self.auth_client.post(self.url, data=self.form_data)
        # Проверяем, что редирект привёл к разделу с комментами.
        self.assertRedirects(respounse, f'{self.url}#comments')
        comments_count = Comment.objects.count()
